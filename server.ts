...
const apkUpload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 100 * 1024 * 1024 } }); // 100MB limit
...
app.post('/api/admin/releases', requireAdmin, apkUpload.single('apk'), async (req, res) => {
  try {
...
app.put('/api/admin/releases/:id', requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
